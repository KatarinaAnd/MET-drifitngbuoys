...
def simba10():
    ...
    gps_ds['time1'].attrs['units'] = 'seconds since 1970-01-01 00:00:00'
    ...
    ds['baro_temp'] = ds.baro_temp.where(ds['baro_temp'] < 30)           #filtering values to stay below 30 degrees
    ...
    ds['baro_temp'].encoding['units'] = 'DegC'
    ds['baro_temp'].encoding['_FillValue'] = -9999
    
    
    
    ds['pressure'].attrs['long_name'] = 'atmospheric air pressure'
    ds['pressure'] = ds.pressure.where(ds['pressure'] < 1050)           #filtering values to stay below 1050 hPa

    ds['pressure'].attrs['standard_name'] = 'air_pressure'
    ds['pressure'].attrs['units'] = 'Pa'
    ds['pressure'].attrs['coverage_content_type'] = 'physicalMeasurement'
    
    ds['pressure'].encoding['units'] = 'Pa'
    ds['pressure'].encoding['_FillValue'] = -9999    
    ...
    ds['mag_x'].encoding['units'] = '1e-9 T'
    ds['mag_x'].encoding['_FillValue'] = -9999
    ...
    ds['mag_y'].encoding['units'] = '1e-9 T'
    ds['mag_y'].encoding['_FillValue'] = -9999
    ...
    ds['acc_x'].encoding['units'] = 'standard_free_fall'
    ds['acc_x'].encoding['_FillValue'] = -9999
    ...
    ds['acc_y'].encoding['units'] = 'standard_free_fall'
    ds['acc_y'].encoding['_FillValue'] = -9999
    ...
    ds['acc_z'].encoding['units'] = 'standard_free_fall'
    ds['acc_z'].encoding['_FillValue'] = -9999

    ds['air_temp'].attrs['long_name'] = 'Air temperature from in-air sensor 240'
    ds['air_temp'] = ds.air_temp.where(ds['air_temp'] < 30)  #filtering values to stay below 30 degrees
    ...
    ds['air_temp'].encoding['units'] = 'degC'
    ds['air_temp'].encoding['_FillValue'] = -9999
        
    ds['time2'].attrs['long_name'] = 'time'
    ds['time2'].attrs['standard_name'] = 'time'
    ds['time2'].attrs['coverage_content_type'] = 'referenceInformation'
    ds['time2'].attrs['units'] = 'seconds since 1970-01-01 00:00:00'
    ...
    ds.attrs['title'] = 'Data from drifting buoy SIMBA 300234065765510'
    ds.attrs['id'] = 'SIMBA 300234065765510'
    ds.attrs['naming_authority'] = 'UiT'
    ds.attrs['summary'] = 'Data from drifting buoys deployed on the 29th of June by UiT, using Kronprins Haakon'
    ds.attrs['source'] = 'Drifitng buoys'
    ...
    ds.attrs['creator_type'] = 'Institution'
...
def set_encoding(ds, fill=-9999, time_name = 'time', time_units='seconds since 1970-01-01 00:00:00'):
    ...
    if 'time2' and 'time1' in ds.coords.keys():
        encoding['time2'] = {'dtype':'i4', '_FillValue' : fill}
        encoding['time1'] = {'dtype':'i4', '_FillValue' : fill}
    elif 'time1' in ds.coords.keys():
        encoding['time1'] = {'dtype': 'i4', '_FillValue': fill}
    else:
        encoding['time'] = {'dtype':'i4', '_FillValue': fill}
    ...
    for data_var in ds.keys():
        ...
        if 'float' in str(ds[data_var].dtype):
            encoding[data_var].update(dtype='float32', _FillValue = fill)
        elif 'int' in str(ds[data_var].dtype):
            encoding[data_var].update(dtype='i4', _FillValue = fill)
    ...
    if parse_arguments().station == 'aws':
        ...
        for group_name, group_da in gb:
            ...
            group_da.attrs['time_coverage_duration'] = ('P' + duration_years + 'Y' + duration_months +
                                                        'M' + duration_days + 'DT' + duration_hours + 
                                                        'H' + duration_minutes + 'M' + duration_seconds + 'S')                    
            group_da.attrs['title'] = ('Buoy aws_300234068660540, from '.format(timestring) + group_da.attrs['time_coverage_start'] +\
                                            ' to ' + group_da.attrs['time_coverage_end'])    
            benedict = group_da.to_dict()
            bad_time1 = benedict['coords']['time']['data']
            benedict['coords']['time']['data'] = np.array([((ti - datetime(1970,1,1)).total_seconds()) for ti in bad_time1]).astype('i4')
            benedict['coords']['longitude']['data'] = np.float32(benedict['coords']['longitude']['data'])
            benedict['coords']['latitude']['data'] = np.float32(benedict['coords']['latitude']['data'])

            group_da = xr.Dataset.from_dict(benedict)
            group_da = group_da.fillna(-9999)

            group_da.to_netcdf('{}/{}'.format(saveto, filename), engine = 'netcdf4', encoding=set_encoding(group_da))
        ...
        if 'time2' in ds.variables:
            ...
            for i in sets:
                ...
                for group_name, group_da in finished:
                    ...
                    group_da.attrs['time_coverage_duration'] = ('P' + duration_years + 'Y' + duration_months +
                                                    'M' + duration_days + 'DT' + duration_hours + 
                                                    'H' + duration_minutes + 'M' + duration_seconds + 'S') 
                    group_da.attrs['title'] = ('Buoy simba_300234065765510, from '.format(timestring) + group_da.attrs['time_coverage_start']+ \
                                                ' to ' + group_da.attrs['time_coverage_end'])  
                    
                    benedict = group_da.to_dict()
                    bad_time1 = benedict['coords']['time1']['data']
                    bad_time2 = benedict['coords']['time2']['data']
                    benedict['coords']['time1']['data'] = np.array([((ti - datetime(1970,1,1)).total_seconds()) for ti in bad_time1]).astype('i4')
                    benedict['coords']['time2']['data'] = np.array([((ti - datetime(1970,1,1)).total_seconds()) for ti in bad_time2]).astype('i4')
                    benedict['coords']['longitude']['data'] = np.float32(benedict['coords']['longitude']['data'])
                    benedict['coords']['latitude']['data'] = np.float32(benedict['coords']['latitude']['data'])
            ...
            for group_name, group_da in gb:
                ...
                group_da.attrs['time_coverage_duration'] = ('P' + duration_years + 'Y' + duration_months +
                                                            'M' + duration_days + 'DT' + duration_hours + 
                                                            'H' + duration_minutes + 'M' + duration_seconds + 'S')                    
                group_da.attrs['title'] = ('Buoy simba_300234065765510_{}.nc, from '.format(timestring) + group_da.attrs['time_coverage_start'] +\
                                                ' to ' + group_da.attrs['time_coverage_end'])
        ...
        if 'time2' in ds.variables:        
            ...
            for i in sets:
                ...
                for group_name, group_da in finished:
                    ...
                    group_da.attrs['time_coverage_duration'] = ('P' + duration_years + 'Y' + duration_months +
                                                    'M' + duration_days + 'DT' + duration_hours + 
                                                    'H' + duration_minutes + 'M' + duration_seconds + 'S')
                    group_da.attrs['title'] = ('Buoy simba_300234065863280, from '.format(timestring) + group_da.attrs['time_coverage_start'] +\
                                                ' to ' + group_da.attrs['time_coverage_end'])
            ...
            for group_name, group_da in gb:
                ...
                group_da.attrs['time_coverage_duration'] = ('P' + duration_years + 'Y' + duration_months +
                                                            'M' + duration_days + 'DT' + duration_hours + 
                                                            'H' + duration_minutes + 'M' + duration_seconds + 'S')                    
                group_da.attrs['title'] = ('Buoy simba_300234065863280_{}.nc, from '.format(timestring) + group_da.attrs['time_coverage_start'] +\
                                                ' to ' + group_da.attrs['time_coverage_end'])            
